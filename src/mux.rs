...
impl Multiplexer {
    ...
    pub fn spawn(&self, ctx: &DaemonContext) -> io::Result<()> {
        ...
        match self {
            ...
            Multiplexer::Nvim => return nvim::spawn(&bin, ctx),
            Multiplexer::Tmux => {
    ...
    pub fn finish(&self, ctx: &DaemonContext, outcome: LoadOutcome) -> io::Result<()> {
        match self {
            Self::Nvim => nvim::finish(ctx, outcome),
            _ => Ok(()),
        }
    }
}
